...
if [ -x /usr/local/bin/k0s ]; then
  ...
  curl -sSLf https://get.k0s.sh | INSTALL_K3S_EXEC="--tls-san 127.0.0.1, 10.0.3.119, fe80::4e1:ecff:fe3b:9bfb, 10.96.0.1 , 35.165.218.54" sh -s -
...
sudo /usr/local/bin/k0s install controller -c k0s.yaml --enable-worker && printf "\n\n${CYAN}K0s cluster has been installed${NC}"
...
kubectl apply -f Dashboard-admin-service-account.yaml && printf "\n\n${CYAN}Dashboard-admin-service-account applied${NC}\n\n"
kubectl apply -f ecr-creds-helper.yaml && printf "\n\n${CYAN}ecr-creds-helper applied${NC}\n\n"
